...
import { ArticleCard, MobCard, VersionCard } from '@/components/cards'
import { articles, mobs, versions } from '@/lib/content'
...
export default function HomePage() {
  return <main>
    ...
    <section className="section dark-section"><div className="section-title-row"><div><span className="eyebrow">Хронология</span><h2>ВАЖНЫЕ ВЕРСИИ</h2></div><Link href="/versions">Все обновления <ArrowRight /></Link></div><div className="versions-layout"><VersionCard version={versions[0]} featured /><div className="stacked-cards">{versions.slice(1, 3).map((v) => <VersionCard key={v.slug} version={v} />)}</div></div></section>
    ...
    <section className="section article-section"><div className="section-title-row"><div><span className="eyebrow">База знаний</span><h2>УЧИСЬ. СТРОЙ. ВЫЖИВАЙ.</h2></div><Link href="/guides">Все гайды <ArrowRight /></Link></div><div className="article-list">{articles.slice(0, 4).map((article, index) => <ArticleCard key={article.slug} article={article} index={index + 1} />)}</div></section>
  </main>
}
